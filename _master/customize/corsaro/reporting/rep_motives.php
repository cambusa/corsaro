...
if(qv_validatesession($maestro, $sessionid, "quiver")){
    ...
    function myheader(){
        ...
        $PAPER->write("<div class='report-title' style='position:absolute;font-size:16px;top:25px;left:0px;width:100%;text-align:center;'>$titlereport</div>");
    };
    ...
    // ESEGUO LA QUERY
    maestro_query($maestro, "SELECT * FROM QVMOTIVES WHERE SYSID IN ($in)", $r);
    for($i=0; $i<count($r); $i++){
        $SYSID=$r[$i]["SYSID"];
        // DETERMINAZIONE TIPOLOGIA
        $TYPOLOGYID=$PAPER->getvalue($r, $i, "TYPOLOGYID");
        maestro_query($maestro, "SELECT DESCRIPTION FROM QVMOTIVETYPES WHERE SYSID='$TYPOLOGYID'", $s);
        $TYPOLOGY=$PAPER->getvalue($s, 0, "DESCRIPTION");
        unset($s);
        // DETERMINAZIONE TIPO GENERE
        $GENRETYPEID=$PAPER->getvalue($r, $i, "GENRETYPEID");
        maestro_query($maestro, "SELECT DESCRIPTION FROM QVGENRETYPES WHERE SYSID='$GENRETYPEID'", $s);
        $GENRETYPE=$PAPER->getvalue($s, 0, "DESCRIPTION");
        unset($s);
        // STAMPA RIGA
