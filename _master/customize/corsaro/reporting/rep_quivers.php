...
if(qv_validatesession($maestro, $sessionid, "quiver")){
    ...
    // ESEGUO LA QUERY
    maestro_query($maestro, "SELECT * FROM QVQUIVERS WHERE SYSID IN ($in)", $r);
    for($i=0; $i<count($r); $i++){
        $SYSID=$r[$i]["SYSID"];
        // DETERMINAZIONE TIPOLOGIA
        $TYPOLOGYID=$PAPER->getvalue($r, $i, "TYPOLOGYID");
        maestro_query($maestro, "SELECT DESCRIPTION FROM QVQUIVERTYPES WHERE SYSID='$TYPOLOGYID'", $s);
