...
if(qv_validatesession($maestro, $sessionid, "quiver")){
    ...
    function myheader(){
        ...
        $PAPER->write("<div class='report-title' style='position:absolute;font-size:16px;top:25px;left:0px;width:100%;text-align:center;'>TIPI QUIVER</div>");
    };
    ...
    // ESEGUO LA QUERY
    maestro_query($maestro, "SELECT * FROM QVQUIVERTYPES WHERE SYSID IN ($in)", $r);
    for($i=0; $i<count($r); $i++){
        $SYSID=$r[$i]["SYSID"];
        maestro_query($maestro, "SELECT FIELDNAME FROM QVQUIVERVIEWS WHERE TYPOLOGYID='$SYSID'", $d);
        if(count($d)>0)
