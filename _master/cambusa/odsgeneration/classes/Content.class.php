...
class Content extends XMLDocument {
			...
			foreach ($cells as $row => $obj) {
				...
				foreach ($obj as $col => $cell) {
					...
					if ($cell->getType() == 'float') {
						$tbCell->setAttribute('office:value-type', 'float');
						$tbCell->setAttribute('office:value', $content);
					}
					elseif ($cell->getType() == 'date') {
                        $content=substr($content,0,10);
                        if($content!="1900-01-01" && $content!="9999-12-31"){
                            $tbCell->setAttribute('style:date-style', 'DD/MM/YY');
                            $tbCell->setAttribute('office:value-type', 'date');
                            $tbCell->setAttribute('office:date-value', $content);
                        }
                        else{
                            $content="";
                            $tbCell->setAttribute('office:value-type', 'string');
                        }
					}
                    else{
                        $tbCell->setAttribute('office:value-type', 'string');
					...
					if ($content != '') {
						$this->_addTextElement('p', $content, $tbCell);
					}
					$colCourant = $col;
				}
				$rowCourant = $row;
	...
	protected function _insertStyles($styles) {
		...
		foreach ($styles['cel'] as $elem) {
			...
			if ($elem['styleName'] != '') {
				$s = (!$s) ? $this->_addStyleElement('table-cell-properties', null, $style) : $s;
				$style->setAttribute('style:data-style-name', $elem['styleName']);
			}
		}
	...
	protected function _getUsedCellStyles($cell, &$styles) {
        ...
        */
		...
		elseif($cell->getDecimal()){
            if(!in_array($str, $styles['numused'])) {
                $styles['numused'][] = $str;
                $strdecimal = array('val' => $cell->getDecimal(), 'pos' => 'N'.count($styles['numused']));
            }
		}
