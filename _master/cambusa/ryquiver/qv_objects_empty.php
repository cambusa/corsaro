...
function qv_objects_empty($maestro, $data){
    ...
    try{
        ...
        if($SYSID!=""){
            ...
            if(isset($data["DATE"]))
                ...
                $date=HIGHEST_DATE;
            $res=maestro_unbuffered($maestro, "SELECT SYSID FROM QVOBJECTS WHERE DELETED=1 AND TIMEDELETE<[:DATE($date)]");
        ...
        for($i=0;$i<count($arrdel);$i++){
            $SYSID=$arrdel[$i];
            maestro_query($maestro,"SELECT TYPOLOGYID FROM QVOBJECTS WHERE SYSID='$SYSID'", $r);
            if(count($r)==1){
                ...
                qv_extension($maestro, $data, "QVOBJECT", $SYSID, $TYPOLOGYID, 3);
            }
        }
    }
    catch(Exception $e){
