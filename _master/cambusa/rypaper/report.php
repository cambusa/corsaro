...
class rypaper{
    ...
    public function begindocument(){
        ...
        if($this->pdf){
            ...
            $this->mpdf=new mPDF("UTF-8", array( $this->pagewidth, $this->pageheight), 0, "", $this->marginleft, $this->marginright, $this->margintop, 0,0,0);
        }
        elseif($this->filed){
            $this->filefp=fopen($this->pathfile,"w");
        }
        $this->write("<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML 4.01 Transitional//EN'>\n");
        $this->write("<html>\n");
        $this->write("<head>\n");
        ...
        $this->write(".cellsx{overflow:hidden;white-space:nowrap;}\n");
        $this->write(".celldx{text-align:right;overflow:hidden;white-space:nowrap;}\n");
        $this->write(".cellcx{text-align:center;overflow:hidden;white-space:nowrap;}\n");
        $this->write(".cellax{text-align:align;}\n");
        $this->write(".table-header-row{background-color:gray;height:".$this->tablerowheight."mm;}\n");
        $this->write(".table-header-cell{color:white;font-weight:bold;}\n");
    ...
    public function enddocument(){
        ...
        if($this->pdf){
            ...
            if($this->filed)
                $this->mpdf->Output($this->pathfile, "F");
            else
                $this->mpdf->Output();
        }
        elseif($this->filed){
