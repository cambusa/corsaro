...
****************************************************************************/
function class_qvplutoscript(settings,missing){
    var formid=RYWINZ.addform(this, settings);
    winzProgress(formid);

    var currsysid="";
    var typescript=RYQUE.formatid("0FINSCRIPT00");
    var prefix="#"+formid;
    var flagopen=false;
    ...
    offsety+=30;
    $(prefix+"LB_DEVELOPER").rylabel({left:20, top:offsety, caption:"Script"});
    $(prefix+"DEVELOPER").rytext({left:100, top:offsety, width:350, datum:"C", tag:"DEVELOPER"});
    
    offsety+=30;
    $(prefix+"LB_OPZIONI").rylabel({left:20, top:offsety, caption:"Opzioni"});
    offsety+=25;
    $(prefix+"OPZIONI").ryedit({left:20, top:offsety, width:700, height:200, flat:true, datum:"C", tag:"OPZIONI"});
        ...
        select:function(i,p){
            ...
            if(!flagsuspend){
                ...
                case 2:
                    ...
                    RYQUE.query({
                        sql:"SELECT * FROM QW_FINSCRIPT WHERE SYSID='"+currsysid+"'",
                        ready:function(v){
