...
****************************************************************************/
function class_qvarrowtypes(settings,missing){
    ...
    var oper_new=$(prefix+"oper_new").rylabel({
        ...
        click:function(o){
            ...
            data["GENRETYPEID"]=tx_genretypeid.key(2);
    ...
    var oper_print=$(prefix+"oper_print").rylabel({
        ...
        button:true,
        click:function(o){
            qv_printselected(formid, objgridsel, "@customize/corsaro/reporting/rep_arrowtypes.php")
        }
    });
    ...
    var oper_delete=$(prefix+"oper_delete").rylabel({
        ...
        button:true,
        click:function(o){
            qv_bulkdelete(formid, objgridsel, "arrowtypes");
        }
    });
    ...
    $(prefix+"LB_GENRETYPEID").rylabel({left:20, top:offsety, caption:"Tipo generi"});
    var tx_genretypeid=$(prefix+"GENRETYPEID").rylist({left:120, top:offsety, width:300, datum:"C", tag:"GENRETYPEID"});offsety+=30;
    
    $(prefix+"LB_MOTIVETYPEID").rylabel({left:20, top:offsety, caption:"Tipo motivi"});
    var tx_motivetypeid=$(prefix+"MOTIVETYPEID").rylist({left:120, top:offsety, width:300, datum:"C", tag:"MOTIVETYPEID"});offsety+=30;
    
    $(prefix+"LB_BOWTYPEID").rylabel({left:20, top:offsety, caption:"Tipo partenza"});
    var tx_bowtypeid=$(prefix+"BOWTYPEID").rylist({left:120, top:offsety, width:300, datum:"C", tag:"BOWTYPEID"});offsety+=30;
    
    $(prefix+"LB_TARGETTYPEID").rylabel({left:20, top:offsety, caption:"Tipo arrivo"});
    var tx_targettypeid=$(prefix+"TARGETTYPEID").rylist({left:120, top:offsety, width:300, datum:"C", tag:"TARGETTYPEID"});offsety+=30;
    ...
    var objgriddetails=$(prefix+"griddetails").ryque({
        ...
        solveid:function(o, d){
            currdetailid=d;
            RYQUE.query({
                sql:"SELECT * FROM QVARROWVIEWS WHERE SYSID='"+d+"'",
                ready:function(v){
                    try{
    ...
    var oper_detailsdelete=$(prefix+"oper_detailsdelete").rylabel({
        ...
        button:true,
        click:function(o){
            qv_bulkdelete(formid, objgriddetails, "arrowviews");
        }
    });

    // DEFINIZIONE TAB DOCUMENTI
    var filemanager=new qv_filemanager(this, formid, "QVARROWTYPES");
        ...
        select:function(i,p){
            ...
            if(!flagsuspend){
                ...
                case 2:
                    ...
                    RYQUE.query({
                        sql:"SELECT * FROM QVARROWTYPES WHERE SYSID='"+currsysid+"'",
                        ready:function(v){
                ...
                case 3:
                    ...
                    objgriddetails.query({
                        ready:function(){
                            qv_contextmanagement(context, {sysid:currsysid, table:"QVARROWTYPES", select:"DESCRIPTION", formula:"[=DESCRIPTION]",
                                done:function(d){
                                    context=d;
                                    lb_details_context.caption("Contesto: "+context);
                                }
